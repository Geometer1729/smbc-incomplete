...
impl CharGrid {
    pub fn from_string(input : String) -> CharGrid{
        ...
        CharGrid{height,width,strs}
    }

    fn join(l : CharGrid,r : CharGrid) -> CharGrid {
        let height=max(l.height,r.height);
        let width =l.width + r.width;
        let lstrs = l.strs.iter().cloned().chain(repeat(repeat(' ').take(l.width).collect())).take(height);
        let rstrs = r.strs.iter().cloned().chain(repeat(repeat(' ').take(r.width).collect())).take(height);
        let strs = zip(lstrs,rstrs).map(|(a,b)|a+&b).collect();
        CharGrid{height,width,strs}
    }

    pub fn join_vec(vec : Vec<CharGrid>) -> CharGrid {
        vec.into_iter().fold(Self::from_string("".into()),Self::join)
    }
